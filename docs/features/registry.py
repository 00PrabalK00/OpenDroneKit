...
INDIA_FIRST = [
    ...
    F("eng.semantic", "Semantic understanding engine", "vision", "India: shared engines",
      ...
      "in_progress", ["tests/test_semantic_engine.py",
                      "tests/test_gorakhpur_metric.py",
                      "tests/test_semantic_registration_honesty.py"],
"Runtime and DINOv2/UPerNet architecture are implemented, and TWO heads have now "
      "been trained and measured. Neither ships. "
      "v1 (shared_semantic_dinov2_vitb14) reached mean IoU 0.6128 in validation and then "
      "predicted BUILDING ON EVERY PIXEL of all four India holdout tiles: precision "
      "0.092, recall 1.0, IoU 0.092. The cause was in the corpus rather than the model. "
      "SpaceNet 7 labels buildings and leaves 96.7 per cent of each tile at IGNORE_INDEX, "
      "so cross-entropy never once penalised calling an unlabelled pixel a building, and "
      "on imagery resembling that source the cheapest answer is 'all building'. "
      "v2 fixed the corpus rather than the recipe. An unlabelled pixel in an exhaustively "
      "annotated tile is evidence the class is ABSENT -- not evidence of what the pixel "
      "is -- so it became a penalty on that one class instead of being discarded. "
      "The pathology went with it: predicted building fraction fell from 1.000 to 0.244 "
      "against a labelled 0.092, IoU rose 0.092 -> 0.274, precision 0.092 -> 0.296, and "
      "recall fell 1.0 -> 0.786, which is the point -- the model can finally say no. "
      "Validation held at mean IoU 0.5888 (building 0.890, road 0.772, vegetation 0.844, "
      "water 0.678), slightly below v1 because part of v1's number was earned by the "
      "shortcut. "
      "STILL NOT SHIPPING: 0.296 precision means seven of every ten pixels it calls "
      "building are not. It over-predicts by 2.7x, and a number an inspector would act "
      "on has to survive being read honestly. "
      "CORRECTION, and it changes the diagnosis. This note previously called SpaceNet 7 "
      "'0.5 m satellite imagery' and blamed the corpus for being small. Measured from the "
      "files' own geotransforms over 60 samples of each source, SpaceNet 7 runs 2.91 to "
      "4.77 m per pixel and OpenEarthMap runs 0.20 to 0.50 -- a spread of twenty-four "
      "times across the corpus, not the eightfold error alone. "
      "The loader cropped a fixed 518 pixels from all of it, so one tile covered 104 m of "
      "ground and another 2.5 km, and both were handed to the model as the same thing. A "
      "building filled the frame in one and was a two-pixel blob in the other. That is a "
      "better explanation of predicting building over 0.244 of a frame that is 0.092 "
      "building than 'the corpus is too small' ever was, and it is a defect in the "
      "pipeline rather than a limit of the data. "
      "training/semantic_tiles.py now crops by ground extent and resamples to a single "
      "target_gsd, 0.30 m, chosen at OpenEarthMap's median because this is a drone product "
      "and the fine end is the end that matters. Masks resample by nearest neighbour, "
      "since averaging class 1 with class 3 would invent class 2. "
      "The scale fix is confirmed, and by the harshest available test: v2's own weights "
      "scored through v3's harmonised protocol predict ZERO building pixels. v2 had not "
      "learned what a building looks like, it had learned a four-metre blob prior, and "
      "shown buildings at the scale it claimed to work at it had nothing to say. "
      "THE HOLDOUT THAT PRODUCED ALL OF THE ABOVE COULD NOT FAIL. SpaceNet 7 leaves 96.7 "
      "per cent of each tile at IGNORE_INDEX; the confusion matrix drops ignored pixels "
      "from prediction and target alike, so no negatives survive and a model predicting "
      "building on every pixel scores mean IoU 1.0. That is measured, not argued. Six "
      "rows were gated on a metric blind to the most likely failure of a segmentation "
      "model, and the multiclass part of the claim rested on a corpus that labels one "
      "class. "
      "REPLACED BY GORAKHPUR, and this is the first honest India evidence the project "
      "has. Gorakhpur (26.76N, 83.37E) is the one Indian region in OpenEarthMap, "
      "recovered from xBD imagery OpenEarthMap does not redistribute, exhaustively "
      "annotated at 0.448 m/px -- near the training scale, not 2.91-4.77 m -- and pinned "
      "to test as 49 tiles that were never trainable. On exhaustive annotation a false "
      "positive lands on a labelled pixel and is counted, so the metric CAN fail. "
      "v4 (epoch 35 of 40, cut at Kaggle's session cap) on Gorakhpur, per class IoU: "
      "vegetation 0.913, building 0.621, road 0.366, water 0.003, bare_land 0.000, "
      "background 0.000; pixel accuracy 0.916. v3 for comparison: 0.905, 0.563, 0.312, "
      "0.092, 0.000. "
      "TWO CLASSES OF SIX ARE REGISTERED AS WORKING -- vegetation and building. Road is "
      "recorded as weak and unusable: 0.366 IoU, recall 0.488, it misses half the road it "
      "is shown. Water is recorded as NOT WORKING at 0.003, and it fails in the dangerous "
      "direction -- it predicts nearly the right amount of water (282851 labelled, 282787 "
      "predicted) in the wrong places, so the output looks plausible and is not. "
      "bare_land is dead: zero pixels predicted across all 49 tiles. "
      "v4's larger corpus is what settles this rather than leaving it open. It added 463 "
      "water and 178 bare_land samples; water FELL from 0.092 to 0.003 and bare_land did "
      "not move off zero, while building (+0.059) and road (+0.053) both improved. More "
      "data is not the outstanding fix for the two dead classes. "
      "The mean IoU of 0.317 is not quoted as a result and should not be: it averages a "
      "0.913 class with two that do not work, and reporting it is how a six-class claim "
      "gets made on the strength of two. This row claims two classes and names the other "
      "four with their numbers. "
      "CAVEATS ON THE NUMBERS THEMSELVES: 13.4 per cent of optimiser steps were dropped "
      "as non-finite, cause unknown -- the fp16-overflow-from-the-absent-class-penalty "
      "hypothesis was tested and died on a measured max |grad| of 0.0625 against the ~1e6 "
      "an overflow needs. The last five epochs are not in these weights and the weekly "
      "Kaggle GPU quota is spent. And 49 tiles of ONE city, captured from satellite or "
      "aircraft rather than from a drone at 60 m, is not India: a good number here means "
      "the engine works on Gorakhpur. "
      "Weights registered as shared_semantic_landcover in models/model_registry.json, "
      "usable_labels ['vegetation', 'building'], ONNX parity max_abs_diff 1.2e-04 with "
      "zero label disagreements over 1073296 pixels. "
      "Evidence: docs/holdout/shared_semantic_v4_gorakhpur.json (v4, the row above), "
      "docs/holdout/shared_semantic_v3_gorakhpur.json (v3), "
      "docs/holdout/shared_semantic_v2_scored_at_v3_scale.json (the zero-building proof), "
      "docs/holdout/shared_semantic_india_holdout.json (v1), "
      "docs/holdout/shared_semantic_v2_india_holdout.json (v2), "
      "tests/test_gorakhpur_metric.py (the metric can fail), "
      "tests/test_semantic_scale.py (the scale fix)."),
