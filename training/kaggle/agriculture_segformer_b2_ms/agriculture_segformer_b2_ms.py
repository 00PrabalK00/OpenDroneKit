"""Kaggle kernel: train agriculture_segformer_b2_ms (segmentation_multiclass).
...
CORPUS = Path("/kaggle/input/odk-agriculture-seg-corpus")
...
def main() -> int:
    ...
    if 'training.train_seg_multiclass' == 'training.train_det':
        ensure_ultralytics()
    ensure_repo()
    corpus = resolve_corpus()

    if "training.train_seg_multiclass" == "training.train_shared_semantic":
        ...
        command = [
            sys.executable, "-m", "training.train_seg_multiclass",
            "--config", str(REPO / "training" / "configs" / "agriculture_segformer_b2_ms.yaml"),
            "--corpus", str(corpus / "corpus.json"),
            "--run-dir", str(OUT / "agriculture_segformer_b2_ms"),
            "--source", "github",
        ]
    else:
        command = [
