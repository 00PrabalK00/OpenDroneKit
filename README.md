...
| Shared semantic head | **Trained twice, measured twice, still not shipping.** v1 predicted building on *every pixel* of all four Indian holdout tiles (precision 0.092). The corpus was the cause: SpaceNet 7 labels buildings and leaves 96.7% of each tile ignored, so nothing ever penalised a false building. v2 turned those unlabelled pixels into evidence of ABSENCE rather than discarding them, and the pathology went with it — predicted building fraction **1.000 → 0.244** against a labelled 0.092, IoU **0.092 → 0.274**, and recall fell 1.0 → 0.786, which is the point: it can finally say no. Still over-predicts 2.7×, so seven of ten "building" pixels are wrong and it does not ship. The limit is the corpus, not the loss. `docs/holdout/` has both runs. |
