...
| Trained defect models | **Nine installed.** solar_cell_defect_detector (mAP50 0.884), crack_presence_classifier (balanced accuracy 0.958), rail_obstacle_detector (mAP50 0.824), solar_thermal_anomaly_classifier (balanced accuracy 0.724), rail_corridor_segmentation (IoU 0.681), crack_segmentation (IoU 0.606), corrosion_severity_segmentation (mean IoU 0.577 over four ordinal grades), structural_multiclass_detector (mAP50 0.417), shared_semantic_landcover (**two of its six classes are usable** — see the row below). Each runs through real ONNX weights and reports `model_used` as `onnx:<file>`. Anything without usable weights still reports `heuristic`, never as AI. |
...
| Shared semantic head | **Ships for two classes of six, and the other four are named rather than hidden.** Registered as `shared_semantic_landcover` with `usable_labels` `["vegetation", "building"]`. On Gorakhpur — 49 exhaustively annotated tiles at 0.448 m/px, pinned to test and never trained on — the per-class IoU is vegetation **0.913**, building **0.621**, road **0.366**, water **0.003**, bare_land **0.000**. Road is weak and misses half the road it is shown. Water fails in the dangerous direction: it predicts roughly the right *amount* of water in the wrong *places*, so the raster looks plausible and is not. bare_land predicted zero pixels across all 49 tiles. The mean IoU of 0.317 is not the result and is not quoted as one — it averages a 0.913 class with two dead ones, and `tests/test_semantic_registration_honesty.py` enforces that a class cannot be listed as usable unless the holdout report says so. Getting here took four runs: v1 predicted building on *every pixel* (precision 0.092) because SpaceNet 7 leaves 96.7% of each tile ignored and nothing penalised a false building; v2 turned those pixels into evidence of ABSENCE and cured the pathology; v3 fixed a **24× scale spread** in the corpus, which v2's own weights confirm was the real limit — scored at the harmonised scale, v2 predicts *zero* building pixels, so it had learned a 4 m blob prior rather than what a building looks like; v4 added 21% more training data and settled the dead classes rather than leaving them open — water *fell* from 0.092 to 0.003 despite 463 more water samples. More data is not their fix. `docs/holdout/` has every run. |
| What the semantic numbers do NOT say | The holdout that produced v1 and v2's figures **could not fail**: SpaceNet 7 drops ignored pixels from prediction and target alike, leaving no negatives, so a model predicting building everywhere scores mean IoU **1.0**. That is measured, not argued. Gorakhpur replaced it because exhaustive annotation makes a false positive land on a labelled pixel and count. Even so: 49 tiles of **one** Indian city, captured from satellite or aircraft rather than from a drone at 60 m. A good number there means the engine works on Gorakhpur. The current weights are also epoch 35 of 40 — the run was cut at Kaggle's session cap — and 13.4% of optimiser steps were dropped as non-finite, cause still unknown. |
...
| Flight code against a real autopilot | **Verified in CI, and only there.** Both SITL suites fly against ArduPilot Copter-4.5.7 in a container and pass. They skip under a plain pytest, and a skip counts as no evidence rather than as a pass, so on a laptop `fl.sitl` honestly reads *implemented* — the container's junit report is published by CI and merged into the status computation, so the row is earned by a flight that happened. Run 32853156668 is the first one that closed the loop end to end: both suites flew green, the report was written and uploaded (artifact 9565377649), and the status job merges it through `tools/feature_status.py --extra-report`. Getting there took three fixes that had nothing to do with flying — the suite was failing to collect at all, skipped tests were being counted as passes, and nested test node ids were mangled so no `tests/sitl/...` selector could ever match. The last run before this one flew both tests green and then lost the evidence: the image runs as an unprivileged user and could not write the report into the checkout. This is not decoration: SITL caught our missions putting NAV_TAKEOFF at sequence 0, which MAVLink reserves for home. ArduPilot silently overwrote it and the aircraft would never have taken off. Every mock-based test passed throughout. |
